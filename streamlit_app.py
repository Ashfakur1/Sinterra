...
with tabs[2]:
    st.markdown(
        ...
        "tracking the target properties (the property penalty is "
        "**asymmetric**: MOR is treated as a *minimum* specification so only "
        "falling short is penalised, WA as a *maximum* specification so only "
        "exceeding it is penalised, and shrinkage as a dimensional "
        "specification penalised in both directions), using the **latest** price/CO2 "
